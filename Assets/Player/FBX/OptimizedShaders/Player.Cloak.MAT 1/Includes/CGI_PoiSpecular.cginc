...
float3 calculateSpecular(in float4 albedo)
{
    ...
    finalSpecular *= specularMask;
...

        {
            ...
            finalSpecular1 = calculateNewSpecular(specularMap1.rgb, float(1), realisticAlbedo1, float4(1,1,1,1), metallic1, float(0.75) * specularMap1.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
        }
        ...
        {
            finalSpecular1 = calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.25,0.3,0,1), metallic1, float(1), specularMap1, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
...

        {
            ...
            finalSpecular1 = AnisotropicSpecular(float(0), float(0), float(0.75), float(0), float(1), float(1), float4(1,1,1,1), metallic1, specularMap1, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular1 *= float4(1,1,1,1);
            finalSpecular1 *= poiLight.attenuation;
        }
